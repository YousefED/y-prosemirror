...
Status: **experimental / exploration.** The library `transform` hook + the
`deltaAttributionToFormat` fix are tested and shippable; the BlockNote example
transform (`yhub-blocknote-demo/src/blockTransform.js`) works for the core
cases but has known open edges (see Caveats).
...
the default 1:1 PM↔Y mapping, a *node replace*: the old `blockContent` is kept
...
encoded as duplicable CRDT nodes** (the type as a node *name*, the children
wrapper as a `blockGroup` *node*), and suggestions/concurrency duplicate them.
...
1. **A userland `transform` hook on `syncPlugin`** — a bijective pair of delta
   ...
   schema; only what's *stored* changes. (Library, generic, identity by default.)

2. **A fix in `deltaAttributionToFormat`** — the attribution manager re-surfaces
   a *removed* attribute (from `base`) as a live attr with a `delete`
...
3. **An example BlockNote transform** (`blockTransform.js`) that uses the hook to
   make type changes *attribute* changes and route incompatible changes to a
...
**(b) Container name carries the content-model *class*.** The container node is
...
- **Different class → different name** → the diff *replaces* (two sibling
...
have *interchangeable, mergeable* content, so folding them into one entity is
...
> The structure is otherwise kept **1:1** with ProseMirror — the nested
> `blockGroup` is preserved, not flattened. (An earlier version flattened it to
> merge concurrent child-adds; that made the Y tree non-1:1 and broke structural
> ops — see Caveats §5.4.)

### 3.3 The `deltaAttributionToFormat` fix

`toDeltaDeep(am)` renders the branch *diffed against base*. For an attribute that
...
value plus a `delete` attribution — to *visualize* the removal. But the rendered
...
the suggested node. Crucially it threads a `nodeDeleted` flag so a *whole node
tombstone* (a deleted block) still keeps its attrs (they're part of what's
...
|---|---|---|---|---|
| **paragraph → heading** | same (`inline*`) | *adds* `level`/`isToggleable` | **one block**, `bcType` attr change | ⚠️ only as the new type — the attr change itself is not marked |
| **heading → paragraph** | same | *drops* `level`/`isToggleable` | **one block**; `deltaAttributionToFormat` drops the orphan attrs (no loop) | ⚠️ same |
| **paragraph → quote** | same | *drops* `textAlignment` | **one block** (same as above) | ⚠️ same |
...
| **unindent then type-change** | — | — | **crash** (`applyDelta` Unexpected case) | ❌ open (§5.5) |
...
*drops* within a class are handled by the `deltaAttributionToFormat` fix. The
"two blocks" path is also where the *deleted* block stays a real, editable node
...
turns attribution on *content* into marks but **not attribution on attrs**. To
...
before/after. (Note: this is *visualization* only — accept/reject already work,
because the old value lives on `base`.)

### 5.2 The parent must allow the attribution marks
When a *whole block* is deleted/inserted as a suggestion (the two-block path), the
...
non-1:1 and broke structural ops (§5.4), so it was **removed**. This case is
...
### 5.5 Unindent (or any move) + type-change → crash (open)
Reproduced via diagnostics. A **move** (unindent) is represented in Yjs as
**delete-at-old-position + insert-at-new-position**, and the AM surfaces *both*
copies of the block **with the same block id**. BlockNote targets blocks **by
id**, so a subsequent "change type" command hits the *first* match — the **deleted
tombstone at the old (nested) position** — not the live moved copy. The PM→Y diff
then tries to `modify` the tombstone's nested `blockGroup`, which **does not exist
in the branch Y** (the branch is flat; the nested structure is only in the AM
overlay) → `applyDelta` throws `Unexpected case`.

This is largely **independent of the transform** — it's the move-as-delete+insert
representation ([CAVEATS.md "Position divergence"](./CAVEATS.md)) compounded by
duplicate block ids and *editing a tombstone*. Directions to fix:
- the binding should not emit `modify` ops targeting deletion tombstones (diff
  should treat tombstoned subtrees as non-modifiable), and/or
- moves should not surface duplicate ids, and/or
- BlockNote's id-based commands should target the live copy, not the tombstone.

### 5.6 External Yjs edits / version diffs
...
Named slots make both cardinalities *structural* (one `content`, one `children`),
...
- `src/sync-plugin.js` — `transform` plugin option/state; `toStore`/`toView`
  applied on the PM→Y and Y→PM paths. **Contains a temporary `[y/prosemirror
  DIAG]` try/catch around the PM→Y `applyDelta`** — remove once §5.5 is resolved.
...
- `yhub-blocknote-demo/src/yhub.js` — removed a stale diagnostic block.
